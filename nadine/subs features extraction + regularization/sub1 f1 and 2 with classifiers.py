...
SUBJECT_FOLDER = "C:/Users/nadin/OneDrive/Desktop/archive (1)/RecordedSessions/sub1"
...
if not os.path.isdir(SUBJECT_FOLDER):
    raise FileNotFoundError(
        ...
        "Please verify the path exists and points to your 'sub1' directory."
    )
...
# ✅ Windows path using forward slashes (your sub1)
SUBJECT_FOLDER  = "C:/Users/nadin/OneDrive/Desktop/archive (1)/RecordedSessions/sub1"
...
# ✅ Windows path to your sub1 folder
SUBJECT_FOLDER  = "C:/Users/nadin/OneDrive/Desktop/archive (1)/RecordedSessions/sub1"
...
# ✅ Windows path to sub1 (use forward slashes, include the whole path)
SUBJECT_FOLDER  = "C:/Users/nadin/OneDrive/Desktop/archive (1)/RecordedSessions/sub1"
...
# ====== FEATURE IMPORTANCE PER EEG ELECTRODE ======
import numpy as np

importances = rf.feature_importances_   # importance for every feature

n_channels   = 8                        # EEG1 … EEG8
feats_per_ch = X_feat.shape[1] // n_channels  # how many features for each channel

channel_importance = []
for ch in range(n_channels):
    start = ch * feats_per_ch
    end   = (ch + 1) * feats_per_ch
    mean_imp = float(importances[start:end].mean())
    channel_importance.append((f"EEG{ch+1}", mean_imp))

# sort from most important to least
channel_importance.sort(key=lambda x: x[1], reverse=True)

print("\nAverage importance per electrode:")
for name, val in channel_importance:
    print(f"{name}: {val:.4f}")

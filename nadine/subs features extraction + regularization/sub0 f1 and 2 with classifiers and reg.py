...
############################################################################
#########################################################################
#Regularization + stats features
# -- coding: utf-8 --
"""
EEG Pipeline (STAT FEATURES) + Regularization Comparison
Load → Filter → ICA → Statistical Feature Extraction → Scale → Split
→ Regularization comparison (Strong vs Weak) for:
   - Logistic Regression (L2 via C)
   - SVM RBF (C)
...
def main():
    print("=" * 80)
    print("EEG PIPELINE (STAT FEATURES) → REGULARIZATION COMPARISON")
    print("=" * 80)
    ...
    # =========================================================
    print("\n[4/5] Extracting statistical features...")
    stat_df = compute_statistical_features(X_clean, axis="time")  # your function
    features_df = stat_df.copy()
    features_df["Label"] = Y

    features_df.to_csv("features_stat.csv", index=False)
    print(f"✓ Statistical features: {features_df.shape} (saved: features_stat.csv)")
    ...
    # ✅ Encode labels so all models behave consistently
    le = LabelEncoder()
    y = le.fit_transform(y_raw)
    print("✓ Label encoding:", dict(zip(le.classes_, le.transform(le.classes_))))
    ...
    # ======================================================
    print("\n" + "=" * 80)
    print("REGULARIZATION RESULTS (Accuracy on test set) — STAT FEATURES")
    print("=" * 80)
    for model_name, acc in results.items():
        print(f"{model_name:35s} {acc:.2%}")
